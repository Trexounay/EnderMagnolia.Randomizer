...
kiero::Status::Enum kiero::init(RenderType::Enum _renderType)
{
	...
	if (_renderType != RenderType::None)
	{
		...
		else
		{
			...
			// NOTE: order matters. A DX12 game (e.g. UE5) usually also loads
			// d3d11.dll (D3D11On12, swapchain compositor, subsystems), so we must
			// probe d3d12.dll BEFORE d3d11.dll or Auto would wrongly pick D3D11
			// and hook a vtable that is never presented -> invisible overlay.
